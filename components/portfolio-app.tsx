...
type LiveImportRow = { id: string; rowNumber: number; status: 'supported' | 'unsupported' | 'invalid' | 'duplicate'; message: string | null; normalizedPayload: Record<string, unknown> | null };
type LiveImportSummary = { id: string; status: 'ready_for_review' | 'committed' | 'discarded' | 'undone' | 'failed' | 'staged' | 'processing'; fileName: string; sourceRowCount: number; warningCount: number; createdAt: string; committedAt?: string | null };
...
export function PortfolioApp({ supabaseConfig, apiConfig }: { supabaseConfig?: PublicSupabaseConfig; apiConfig?: PublicApiConfig }) {
  ...
  useEffect(() => {
    ...
    void client.auth.getSession().then(async ({ data }) => {
      ...
      const response = await fetch(`${apiConfig.baseUrl}/v1/accounts/${selectedAccountId}/imports`, { headers: { authorization: `Bearer ${data.session.access_token}` } });
      const payload: unknown = await response.json();
      if (active && response.ok && payload && typeof payload === 'object' && 'imports' in payload && Array.isArray(payload.imports)) setImportHistory(payload.imports as LiveImportSummary[]);
    }).catch(() => undefined).finally(() => { if (active) setHistoryLoading(false); });
    return () => { active = false; };
  ...
  async function persistLiveImport() {
    ...
    try {
      ...
      const payload: unknown = await response.json();
      if (!response.ok || !payload || typeof payload !== 'object' || !('import' in payload)) throw new Error(payload && typeof payload === 'object' && 'error' in payload ? String(payload.error) : 'The server could not stage this CSV.');
      const importId = (payload as { import: { id: string } }).import.id;
      const detailResponse = await fetch(`${apiConfig.baseUrl}/v1/imports/${importId}`, { headers: { authorization: `Bearer ${data.session.access_token}` } });
      const detail: unknown = await detailResponse.json();
      if (!detailResponse.ok || !detail || typeof detail !== 'object' || !('sourceRows' in detail) || !Array.isArray(detail.sourceRows)) throw new Error('The CSV was saved, but its review rows could not be loaded.');
  ...
  async function commitLiveImport() {
    ...
    try {
      const { data } = await client.auth.getSession();
      if (!data.session?.access_token) throw new Error('Your sign-in session has expired. Sign in again before committing.');
      const response = await fetch(`${apiConfig.baseUrl}/v1/imports/${stagedImportId}/commit`, { method: 'POST', headers: { authorization: `Bearer ${data.session.access_token}` } });
      const payload: unknown = await response.json();
      if (!response.ok) throw new Error(payload && typeof payload === 'object' && 'error' in payload ? String(payload.error).replaceAll('_', ' ') : 'The server could not commit this import.');
      setHistoryVersion((current) => current + 1);
      setStageMessage('Import committed. Your reporting data will update after price and report processing are connected.');
    } catch (error) {
      setStageMessage(error instanceof Error ? error.message : 'We could not commit this import.');
    } finally {
      setIsStagingImport(false);
    }
  }
  ...
  async function undoLiveImport(importId: string) {
    if (!client || !apiConfig) throw new Error('Sign in before undoing an import.');
    const { data } = await client.auth.getSession();
    if (!data.session?.access_token) throw new Error('Your sign-in session has expired. Sign in again before undoing an import.');
    const response = await fetch(`${apiConfig.baseUrl}/v1/imports/${importId}/undo`, { method: 'POST', headers: { authorization: `Bearer ${data.session.access_token}` } });
    const payload: unknown = await response.json();
    if (!response.ok || !payload || typeof payload !== 'object' || !('import' in payload)) throw new Error(payload && typeof payload === 'object' && 'error' in payload ? String(payload.error).replaceAll('_', ' ') : 'This import could not be undone.');
    setHistoryVersion((current) => current + 1);
  }
  ...
  return <main className="min-h-screen bg-[#f5f7fb] text-[#13233a]">
    ...
    <div className="mx-auto flex max-w-[1440px]">
      ...
      <section id="top" className="min-w-0 flex-1 px-4 py-7 md:px-8 md:py-10">
        ...
        {active === 'Documents' && <Documents onUpload={openFileChooser} uploadError={uploadError} accounts={accounts} selectedAccountId={selectedAccountId} onSelectAccount={setSelectedAccountId} canStage={Boolean(client && userId && apiConfig && selectedAccountId)} isStaging={isStagingImport} importHistory={importHistory} historyLoading={historyLoading} onUndo={undoLiveImport} />}
      </section>
    </div>
    ...
    <ImportReview open={reviewOpen} fileName={stagedFileName} activities={stagedActivities ?? []} livePreview={livePreview} liveRows={liveRows} stagedImportId={stagedImportId} isStaging={isStagingImport} stageMessage={stageMessage} onStage={persistLiveImport} onCommit={commitLiveImport} onDiscard={clearStaging} />
  </main>;
}
...
function Documents({ onUpload, uploadError, accounts, selectedAccountId, onSelectAccount, canStage, isStaging, importHistory, historyLoading, onUndo }: { onUpload: () => void; uploadError: string; accounts: LiveAccount[]; selectedAccountId?: string; onSelectAccount: (accountId: string) => void; canStage: boolean; isStaging: boolean; importHistory: LiveImportSummary[]; historyLoading: boolean; onUndo: (importId: string) => Promise<void> }) { const live = accounts.length > 0; const [undoError, setUndoError] = useState<string>(); const [undoing, setUndoing] = useState<string>(); const latestCommittedId = importHistory.filter((item) => item.status === 'committed').sort((left, right) => (right.committedAt ?? '').localeCompare(left.committedAt ?? ''))[0]?.id; async function undo(importId: string) { setUndoing(importId); setUndoError(undefined); try { await onUndo(importId); } catch (error) { setUndoError(error instanceof Error ? error.message : 'This import could not be undone.'); } finally { setUndoing(undefined); } } return <><div className="mb-8"><Pill tone={live ? 'green' : 'gold'}>{live ? 'Secure staged import' : 'Local preview only'}</Pill><h1 className="mt-3 text-3xl font-bold tracking-tight">{live ? 'Import activity' : 'CSV preview'}</h1></div><section className="grid min-h-80 place-items-center rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-center"><div><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#eaf2ff] text-[#185da8]"><FileUp size={25}/></span><h2 className="mt-5 text-xl font-bold">{live ? 'Review a Robinhood activity CSV' : 'Preview a Robinhood activity CSV'}</h2>{live ? <><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">The Worker validates this CSV and saves it for your review. Nothing affects your portfolio until you commit the reviewed import.</p><label className="mx-auto mt-5 block max-w-sm text-left text-sm font-semibold text-slate-700">Account<select value={selectedAccountId ?? ''} onChange={(event) => onSelectAccount(event.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm"><option value="" disabled>Select an account</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name} · {accountTypeLabel(account.account_type)}</option>)}</select></label></> : <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">This early prototype reads a selected CSV in your browser only to show a temporary interpretation. It does not upload, store, or add the rows to a portfolio.</p>}<Button disabled={isStaging || (live && !canStage)} onClick={onUpload} className="mt-6 rounded-xl bg-[#185da8] text-white"><Upload size={16}/>{isStaging ? 'Reading CSV…' : 'Choose CSV'}</Button>{uploadError && <p role="alert" className="mx-auto mt-4 max-w-md rounded-xl bg-amber-50 p-3 text-sm text-amber-800">{uploadError}</p>}<p className="mt-4 text-xs text-slate-400">CSV only · Up to 10 MB · {live ? 'Server-side validation and review' : 'Temporary browser preview'}</p></div></section>{live && <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-4"><h2 className="text-lg font-bold">Import history</h2><p className="mt-1 text-sm text-slate-500">Only the latest committed import can be undone.</p></div>{undoError && <p role="alert" className="mb-4 rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{undoError}</p>}{historyLoading ? <p className="text-sm text-slate-500">Loading imports…</p> : importHistory.length === 0 ? <p className="text-sm text-slate-500">No imports for this account yet.</p> : <div className="space-y-3">{importHistory.map((item) => <div key={item.id} className="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-100 p-4"><div className="min-w-40 flex-1"><p className="font-semibold">{item.fileName}</p><p className="mt-0.5 text-xs text-slate-500">{item.sourceRowCount} rows · {item.warningCount} warnings</p></div><Pill tone={item.status === 'committed' ? 'green' : item.status === 'ready_for_review' ? 'gold' : 'slate'}>{item.status.replaceAll('_', ' ')}</Pill>{item.id === latestCommittedId && <Button disabled={undoing === item.id} onClick={() => void undo(item.id)} className="rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200">{undoing === item.id ? 'Undoing…' : 'Undo'}</Button>}</div>)}</div>}</section>}</>; }

function ImportReview({ open, fileName, activities, livePreview, liveRows, stagedImportId, isStaging, stageMessage, onStage, onCommit, onDiscard }: { open: boolean; fileName: string; activities: Activity[]; livePreview?: LiveImportPreview; liveRows: LiveImportRow[]; stagedImportId?: string; isStaging: boolean; stageMessage?: string; onStage: () => Promise<void>; onCommit: () => Promise<void>; onDiscard: () => void }) {
  ...
  const canCommit = Boolean(stagedImportId && review && !livePreview?.duplicateFile && review.acceptedRowCount > 0 && liveWarnings === 0);
  return <Dialog open={open} onOpenChange={(nextOpen) => { if (!nextOpen) onDiscard(); }}><DialogContent className="max-h-[90vh] max-w-2xl overflow-auto rounded-3xl bg-white p-6 shadow-2xl md:p-8"><DialogHeader><Pill tone={livePreview ? 'green' : 'gold'}>{livePreview ? 'Server-side review' : 'Temporary browser preview'}</Pill><DialogTitle className="mt-3 text-2xl font-bold">Review parsed rows</DialogTitle><DialogDescription className="text-sm text-slate-500">{livePreview ? `${fileName || 'Selected CSV'} has been parsed securely but is not yet committed to your portfolio.` : `${fileName || 'Selected CSV'} was not uploaded or saved. These rows cannot affect the demo portfolio.`}</DialogDescription></DialogHeader><div className="grid grid-cols-3 gap-3"><Metric label="Rows found" value={String(review?.sourceRowCount ?? activities.length)}/><Metric label="Interpreted" value={String(review?.acceptedRowCount ?? activities.length - warnings)}/><Metric label="Needs review" value={String(review ? liveWarnings : warnings)}/></div>{livePreview?.duplicateFile && <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800">This exact file was already imported for this account.</p>}{(liveWarnings > 0 || warnings > 0) && <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800">Some rows need review. They remain visible and cannot silently change reports.</p>}{liveRows.length > 0 && <div className="max-h-52 overflow-auto rounded-xl border border-slate-200"><table className="w-full text-left text-sm"><thead className="sticky top-0 bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-3">Row</th><th className="p-3">Status</th><th className="p-3">Activity</th><th className="p-3">Message</th></tr></thead><tbody>{liveRows.slice(0, 50).map((row) => <tr key={row.id} className="border-t border-slate-100"><td className="p-3">{row.rowNumber}</td><td className="p-3"><Pill tone={row.status === 'supported' ? 'green' : 'gold'}>{row.status}</Pill></td><td className="p-3">{typeof row.normalizedPayload?.type === 'string' ? row.normalizedPayload.type : '—'}</td><td className="p-3 text-slate-500">{row.message ?? '—'}</td></tr>)}</tbody></table></div>}{!livePreview && <div className="max-h-52 overflow-auto rounded-xl border border-slate-200"><table className="w-full text-left text-sm"><thead className="sticky top-0 bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-3">Date</th><th className="p-3">Type</th><th className="p-3">Symbol</th></tr></thead><tbody>{activities.slice(0, 20).map((item) => <tr key={item.id} className="border-t border-slate-100"><td className="p-3">{item.date}</td><td className="p-3">{item.kind}</td><td className="p-3">{item.symbol ?? '—'}</td></tr>)}</tbody></table></div>}{stageMessage && <p role="status" className="rounded-xl bg-emerald-50 p-3 text-sm text-emerald-800">{stageMessage}</p>}<DialogFooter className="mt-3 rounded-b-2xl">{livePreview && !stagedImportId && <Button disabled={isStaging || livePreview.duplicateFile} onClick={() => void onStage()} className="rounded-xl bg-[#185da8] text-white">{isStaging ? 'Saving…' : 'Save for review'} <ChevronRight size={16}/></Button>}{canCommit && !stageMessage && <Button disabled={isStaging} onClick={() => void onCommit()} className="rounded-xl bg-[#185da8] text-white">{isStaging ? 'Committing…' : 'Commit import'} <ChevronRight size={16}/></Button>}<Button onClick={onDiscard} className="rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200">{stageMessage ? 'Done' : 'Discard preview'}</Button></DialogFooter></DialogContent></Dialog>;
}
