...
export function composePersistedReportInputs(input: {
  ...
  const closes = input.closes
    ...
    .sort((left, right) => left.tradingDate.localeCompare(right.tradingDate) || left.instrumentId.localeCompare(right.instrumentId) || left.sourceRevision.localeCompare(right.sourceRevision));
