...
export function summarizeOperationalStatus(input: {
  ...
  if (market?.lastSuccessfulAt) {
    if (!Number.isFinite(market.lastSuccessfulAt.getTime())) throw new Error('Market-data success timestamp must be valid.');
    if (market.lastSuccessfulAt > now) throw new Error('Market-data success timestamp cannot be in the future.');
  }
  ...
  if (recovery?.lastBackupAt) {
    if (!Number.isFinite(recovery.lastBackupAt.getTime())) throw new Error('Backup timestamp must be valid.');
    if (recovery.lastBackupAt > now) throw new Error('Backup timestamp cannot be in the future.');
  }
