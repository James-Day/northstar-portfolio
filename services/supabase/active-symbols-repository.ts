...
export type SupabaseActiveSymbolsRepositoryOptions = {
  supabaseUrl: string;
...
export class SupabaseActiveSymbolsRepository {
  ...
  constructor(
    private readonly options: SupabaseActiveSymbolsRepositoryOptions,
  ) {
    this.baseUrl = new URL(options.supabaseUrl);
    if (!options.serviceRoleKey.trim())
      throw new Error(
        "SUPABASE_SERVICE_ROLE_KEY is required for active-symbol discovery.",
      );
  ...
  async list(): Promise<string[]> {
    ...
    if (!Number.isInteger(pageSize) || pageSize < 1 || pageSize > 5000)
      throw new Error(
        "Active-symbol page size must be an integer from 1 through 5000.",
      );
    const lots: Array<z.infer<typeof lotSchema>> = [];
    for (let offset = 0; ; offset += pageSize) {
      ...
      lotsUrl.searchParams.set("order", "instrument_id.asc");
      lotsUrl.searchParams.set("limit", String(pageSize));
      lotsUrl.searchParams.set("offset", String(offset));
      const lotsResponse = await this.fetcher(lotsUrl, {
        headers: this.headers(),
      });
      if (!lotsResponse.ok)
        throw new Error(
          `Supabase active-lot query failed with HTTP ${lotsResponse.status}.`,
        );
      const page = z.array(lotSchema).parse(await lotsResponse.json());
      lots.push(...page);
    ...
    aliasesUrl.searchParams.set(
      "instrument_id",
      "in.(" + instrumentIds.join(",") + ")",
    );
    const aliasesResponse = await this.fetcher(aliasesUrl, {
      headers: this.headers(),
    });
    if (!aliasesResponse.ok)
      throw new Error(
        `Supabase instrument-alias query failed with HTTP ${aliasesResponse.status}.`,
      );
