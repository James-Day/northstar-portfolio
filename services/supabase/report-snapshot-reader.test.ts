...
describe('Supabase report snapshot reader', () => {
  ...
  it('fails closed if an account report response is broader than the requested account', async () => {
    const fetcher = vi.fn().mockResolvedValue(new Response(JSON.stringify([{ id: '11111111-1111-4111-8111-111111111111', user_id: '22222222-2222-4222-8222-222222222222', account_id: '44444444-4444-4444-8444-444444444444', report_type: 'account_daily', as_of_date: '2026-07-06', import_state_revision: 'rev-1', price_revision_id: null, payload: { totalValue: '999' }, published_at: '2026-07-06T23:00:00Z' }])));
    const reader = new SupabaseReportSnapshotReader({ supabaseUrl: 'https://supabase.test', anonKey: 'public-key', fetcher: fetcher as typeof fetch });
    await expect(reader.getLatest('33333333-3333-4333-8333-333333333333', 'user-token')).resolves.toBeUndefined();
  });

  it('fails closed if a consolidated response has another user or account scope', async () => {
    const fetcher = vi.fn().mockResolvedValue(new Response(JSON.stringify([{ id: '11111111-1111-4111-8111-111111111111', user_id: '44444444-4444-4444-8444-444444444444', account_id: null, report_type: 'consolidated_daily', as_of_date: '2026-07-06', import_state_revision: 'rev-1', price_revision_id: null, payload: { totalValue: '999' }, published_at: '2026-07-06T23:00:00Z' }])));
    const reader = new SupabaseReportSnapshotReader({ supabaseUrl: 'https://supabase.test', anonKey: 'public-key', fetcher: fetcher as typeof fetch });
