...
import type { BillingWebhook, Entitlement, EntitlementStatus } from '@/services/billing/entitlements';

const customerSchema = z.object({
  user_id: z.string().uuid(),
  entitlement_status: z.enum(['inactive', 'trialing', 'active', 'past_due', 'canceled']),
...
export class SupabaseBillingRepository implements BillingPersistence {
  ...
  constructor(private readonly options: SupabaseBillingRepositoryOptions) {
    ...
    if (!options.supabaseAnonKey.trim()) throw new Error('SUPABASE_ANON_KEY is required for billing reads.');
    this.fetcher = options.fetcher ?? ((input, init) => fetch(input, init));
  }

  async getEntitlement(userId: string, accessToken: string): Promise<Entitlement | undefined> {
    const url = new URL('/rest/v1/billing_customers', this.baseUrl);
    url.searchParams.set('user_id', `eq.${userId}`);
    url.searchParams.set('select', 'user_id,entitlement_status,trial_started_at,trial_ends_at,last_webhook_created_at,last_webhook_id');
    url.searchParams.set('limit', '1');
    const response = await this.fetcher(url, { headers: this.userHeaders(accessToken) });
    if (!response.ok) throw new Error(`Supabase billing entitlement query failed with HTTP ${response.status}.`);
    const rows = z.array(customerSchema).parse(await response.json());
    if (!rows[0]) return undefined;
    return mapCustomer(rows[0], userId);
  }

  async startTrialAfterCommittedImport(userId: string, accessToken: string): Promise<Entitlement | undefined> {
    const response = await this.rpc('start_trial_after_committed_import', {}, this.userHeaders(accessToken));
    const rows = z.array(customerSchema).parse(await response.json());
    if (!rows[0]) return this.getEntitlement(userId, accessToken);
    return mapCustomer(rows[0], userId);
  }

  async applyVerifiedWebhook(userId: string, event: BillingWebhook, payload: Record<string, unknown> = {}): Promise<Entitlement> {
    if (!this.options.serviceRoleKey?.trim()) throw new Error('SUPABASE_SERVICE_ROLE_KEY is required for billing webhook writes.');
    const response = await this.rpc('apply_billing_webhook', {
      p_user_id: userId,
      p_event_id: event.id,
    ...
    const rows = z.array(customerSchema).parse(await response.json());
    if (!rows[0]) throw new Error('Supabase did not return the billing entitlement after webhook application.');
    return mapCustomer(rows[0], userId);
  }
  ...
  private async rpc(name: string, body: Record<string, unknown>, headers: Record<string, string>) {
    const response = await this.fetcher(new URL(`/rest/v1/rpc/${name}`, this.baseUrl), {
      method: 'POST', headers: { ...headers, 'content-type': 'application/json' }, body: JSON.stringify(body),
    });
    if (!response.ok) throw new Error(`Supabase billing RPC ${name} failed with HTTP ${response.status}.`);
    return response;
  }
}

function mapCustomer(row: z.infer<typeof customerSchema>, expectedUserId: string): Entitlement {
  if (row.user_id !== expectedUserId) throw new Error('Supabase returned a billing customer for another user.');
  return {
    ...
    processedWebhookIds: [],
    lastWebhookCreatedAt: row.last_webhook_created_at ? new Date(row.last_webhook_created_at) : null,
