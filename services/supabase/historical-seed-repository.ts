...
export class SupabaseHistoricalSeedRepository implements HistoricalSeedJobRepository {
  ...
  async recordPage(input: { jobId: string; sourceRevision: string; cursor: DoltHubCloseCursor | null; accepted: HistoricalPriceUpsert[]; mappings: HistoricalSeedMapping[]; quarantined: HistoricalSeedQuarantine[]; continuityIssues: HistoricalContinuityIssue[]; upserted: number }): Promise<void> {
