...
describe('standalone API', () => {
  const trialEntitlement = (
    ...
    trialEndsAt: new Date('2099-01-15T00:00:00Z'),
    processedWebhookIds: [],
  ...
  it('enforces the API rate-limit boundary before route work and returns safe headers', async () => {
    ...
    const limited = await createApi({ rateLimitStore: limitedStore }).request(
  ...
  it('returns a truthful authenticated billing status', async () => {
    const app = createApi({
      verifySession: async () => ({ id: 'user-123' }),
      billingPersistence: {
        getEntitlement: async () => trialEntitlement(),
        startTrialAfterCommittedImport: async () => trialEntitlement(),
        applyVerifiedWebhook: async () => trialEntitlement(),
      },
    });
    const response = await app.request('http://api.test/v1/billing/status', {
      headers: { authorization: 'Bearer session-token' },
    });
    expect(response.status).toBe(200);
  ...
  it('blocks persisted report reads after cancellation while leaving billing status explicit', async () => {
    const app = createApi({
      ...
      billingPersistence: {
        ...
        startTrialAfterCommittedImport: async () =>
          trialEntitlement('canceled'),
        applyVerifiedWebhook: async () => trialEntitlement('canceled'),
      },
    });
    const response = await app.request(
      'http://api.test/v1/accounts/account-1/report',
      { headers: { authorization: 'Bearer session-token' } },
    );
  ...
  it('accepts a signed Stripe webhook using the untouched request body and injects processing', async () => {
    ...
    const app = createApi({
      billing: {
        ...
        createBillingPortalSession: async () => ({ url: '' }),
    ...
    const response = await app.request(
      'http://api.test/v1/billing/stripe/webhook',
      {
        method: 'POST',
        headers: {
  ...
  it('creates an authenticated checkout session from a server-selected price', async () => {
    ...
    const response = await app.request(
      'http://api.test/v1/billing/checkout',
      {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          plan: 'monthly',
          priceId: 'price_attacker_value',
        }),
      },
      {
        APP_ORIGIN: 'https://app.example.com',
        STRIPE_MONTHLY_PRICE_ID: 'price_monthlytest',
      },
    );
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
  ...
  it('creates an authenticated billing portal session using the same app origin', async () => {
    ...
    const response = await app.request(
      'http://api.test/v1/billing/portal',
    ...
    expect(received).toEqual({
      userId: 'user-123',
      accessToken: 'session-token',
      returnUrl: 'https://app.example.com/dashboard?billing=cancelled',
    });
  });
  ...
  it('rejects invalid plans, missing configured prices and unsafe session URLs', async () => {
    const billing = {
      createCheckoutSession: async () => ({
        url: 'http://unsafe.example.test/session',
      }),
      createBillingPortalSession: async () => ({
        url: 'https://billing.stripe.com/p/session_test',
      }),
    ...
    const missingPrice = await app.request(
      'http://api.test/v1/billing/checkout',
      {
        method: 'POST',
        headers: {
  ...
  it('allows the configured app origin to stage authenticated browser requests without opening the API to other origins', async () => {
    ...
    const allowed = await app.request(
      ...
      { APP_ENV: 'production', APP_ORIGIN: 'https://app.example.com' },
    );
  ...
  it('serves an account-scoped freshness report only after session verification', async () => {
    const get = async (accountId: string, userId: string, token: string) => {
      expect([accountId, userId, token]).toEqual([
        'account-123',
        'user-123',
        'session-token',
      ]);
      return {
        expectedDate: '2026-07-06' as never,
        rows: [
          {
            symbol: 'AAPL',
            expectedDate: '2026-07-06' as never,
    ...
    const app = createApi({
      verifySession: async () => ({ id: 'user-123' }),
      priceFreshnessRepository: { get },
    });
    const response = await app.request(
      'http://api.test/v1/accounts/account-123/price-freshness',
      { headers: { authorization: 'Bearer session-token' } },
    );
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
  ...
  it('serves the latest persisted report snapshot through the authenticated route', async () => {
    const app = createApi({
      ...
      reportSnapshotReader: {
        getLatest: async (accountId, token) => {
          ...
          return {
            ...
            asOfDate: '2026-07-06',
            importStateRevision: 'rev-1',
            priceRevisionId: null,
            payload: { totalValue: '100' },
            publishedAt: '2026-07-06T23:00:00Z',
          };
        },
      },
    });
    const response = await app.request(
      'http://api.test/v1/accounts/account-123/report',
      { headers: { authorization: 'Bearer session-token' } },
    );
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      snapshot: { asOfDate: '2026-07-06', payload: { totalValue: '100' } },
    });
  });
  ...
  it('serves paginated account activity with source-row detail after ownership verification', async () => {
    const list = async (
      ...
      expect([accountId, token, input]).toEqual([
        'account-123',
        'session-token',
        { limit: 10, offset: 20 },
      ]);
    ...
    const app = createApi({
      ...
      accountsRepository: {
        list: async () => [],
        get: async () => ({ id: 'account-123' }) as never,
        create: async () => {
    ...
    const response = await app.request(
      'http://api.test/v1/accounts/account-123/activity?limit=10&offset=20',
      { headers: { authorization: 'Bearer session-token' } },
    );
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
      activity: { items: [], limit: 10, offset: 20, hasMore: false },
    });
  });
  ...
  it('exports account activity as formula-safe CSV', async () => {
    ...
    const response = await app.request(
      'http://api.test/v1/accounts/account-123/activity.csv',
      { headers: { authorization: 'Bearer session-token' } },
    );
    expect(response.status).toBe(200);
    expect(response.headers.get('content-type')).toContain('text/csv');
  ...
  it('walks every activity page for a complete export', async () => {
    ...
    const activity = (id: string, description: string) => ({
      id,
      accountId: 'account-123',
      effectiveDate: '2026-01-01',
      entryType: 'buy',
    ...
    const app = createApi({
      ...
      activityRepository: {
        list: async (_accountId, _token, input) => {
          calls.push(input ?? {});
          if (input?.offset === 0)
            return {
              items: [
                activity('11111111-1111-4111-8111-111111111111', 'first'),
              ],
              limit: 100,
              offset: 0,
              hasMore: true,
          ...
          return {
            items: [
              activity('22222222-2222-4222-8222-222222222222', '+second'),
            ],
            limit: 100,
            offset: 1,
            hasMore: false,
  ...
  it('exports an owned persisted report as a rectangular CSV', async () => {
      ...
      reportSnapshotReader: {
        getLatest: async () => ({
          ...
          payload: {
            ...
            activityCoveredThrough: '2026-07-05',
            pricesThrough: '2026-07-06',
            holdings: [
              { instrumentId: 'AAPL', quantity: '1', close: '80', value: '80' },
            ],
          },
        }),
      },
    ...
    const response = await app.request(
      'http://api.test/v1/accounts/account-123/report.csv',
      { headers: { authorization: 'Bearer session-token' } },
    );
    expect(response.status).toBe(200);
    expect(response.headers.get('content-disposition')).toContain(
      'portfolio-report-account-123.csv',
    );
  ...
  it('keeps account deletion request unavailable until a durable executor is configured', async () => {
    ...
    const response = await app.request(
      'http://api.test/v1/me/deletion-request',
      { method: 'POST', headers: { authorization: 'Bearer session-token' } },
    );
  ...
  it('creates only a supported Robinhood account for the verified user', async () => {
    const create = async (
      ...
      token: string,
      input: {
        accountType: 'individual' | 'traditional_ira' | 'roth_ira';
        name: string;
      },
    ) => {
      expect(userId).toBe('user-123');
    ...
    await expect(response.json()).resolves.toMatchObject({
      account: { id: 'account-123', accountType: 'roth_ira' },
    });
  });
  ...
  it('reads and saves owned opening history through the authenticated API', async () => {
    ...
    let saved: unknown;
    const history = parseOpeningHistory({
      openingCash: '125.5',
      activityCoveredFrom: null,
      incompleteReason: 'History begins after account opening.',
      positions: [
        {
          instrumentId: 'vti',
          quantity: '2.5',
    ...
    const app = createApi({
      ...
      openingHistoryRepository: {
        get: async (accountId, userId, token) => {
          expect([accountId, userId, token]).toEqual([
            account.id,
            'user-123',
            'session-token',
        ...
        save: async (accountId, userId, token, value) => {
          expect([accountId, userId, token]).toEqual([
            account.id,
            'user-123',
            'session-token',
    ...
    const read = await app.request(
      `http://api.test/v1/accounts/${account.id}/opening-history`,
      { headers },
    );
    ...
    const write = await app.request(
      `http://api.test/v1/accounts/${account.id}/opening-history`,
      { method: 'PUT', headers, body: JSON.stringify(history) },
    );
  ...
  it('parses a CSV only after the verified user owns the selected account, then checks its hash', async () => {
    ...
    const response = await app.request(
      `http://api.test/v1/accounts/${account.id}/import-preview`,
      {
        method: 'POST',
        headers: {
          authorization: 'Bearer session-token',
  ...
  it('does not preview an import for an account that RLS did not return', async () => {
    const app = createApi({
      ...
      importsRepository: {
        hasFileHash: async () => {
          throw new Error('must not query imports');
        },
        stage: async () => {
          throw new Error('must not stage imports');
        },
        list: async () => [],
        get: async () => undefined,
        discard: async () => undefined,
    ...
    const response = await app.request(
      'http://api.test/v1/accounts/another-user-account/import-preview',
      {
        method: 'POST',
        headers: {
          authorization: 'Bearer session-token',
  ...
  it('stages owned CSV rows through the atomic repository after duplicate-file preflight', async () => {
    ...
    const stage = async (
      token: string,
      input: { fileName: string; accountId: string },
    ) => {
      expect(token).toBe('session-token');
    ...
    const response = await app.request(
      `http://api.test/v1/accounts/${account.id}/imports`,
      {
        method: 'POST',
        headers: {
          authorization: 'Bearer session-token',
          'content-type': 'text/csv',
          'x-file-name': 'activity.csv',
        },
        body: 'Activity Date,Trans Code,Amount\n2026-01-02,Interest,$2',
      },
    );
  ...
  it('lists an account import history only after account ownership is confirmed', async () => {
    ...
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      imports: [{ id: 'import-123', status: 'ready_for_review' }],
    });
  });
  ...
  it('discards an RLS-authorized review import without deleting it', async () => {
    ...
    const response = await app.request(
      'http://api.test/v1/imports/import-123/discard',
      { method: 'POST', headers: { authorization: 'Bearer session-token' } },
    );

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      import: { id: 'import-123', status: 'discarded' },
    });
  });
  ...
  it('commits a reviewed import through the authenticated repository', async () => {
    ...
    const response = await app.request(
      'http://api.test/v1/imports/import-123/commit',
      { method: 'POST', headers: { authorization: 'Bearer session-token' } },
    );
  ...
  it('reports stored review issues as a resolvable conflict', async () => {
    ...
    expect(response.status).toBe(409);
    await expect(response.json()).resolves.toEqual({
      error: 'review_issues_must_be_resolved',
    });
  });
  ...
  it('undoes the latest committed import through the authenticated repository', async () => {
    ...
    const response = await app.request(
      'http://api.test/v1/imports/import-123/undo',
      { method: 'POST', headers: { authorization: 'Bearer session-token' } },
    );

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      import: { id: 'import-123', status: 'undone' },
    });
  });
  ...
  it('returns persisted review rows through the RLS-scoped import repository', async () => {
        ...
        sourceRows: [
          {
            ...
            rowNumber: 2,
            raw: { amount: '$2' },
            normalizedPayload: { type: 'interest' },
    ...
    const response = await app.request(
      'http://api.test/v1/imports/import-123',
      { headers: { authorization: 'Bearer session-token' } },
    );
  ...
  it('creates a signed upload URL only for the authenticated account', async () => {
    const create = async (
      ...
      expect([userId, token, accountId, fileName]).toEqual([
        'user-123',
        'session-token',
        'account-123',
        'activity.csv',
      ...
      return {
        bucket: 'brokerage-statements' as const,
        path: 'user-123/account-123/upload.csv',
        token: 'upload-token',
    ...
    const app = createApi({
      verifySession: async () => ({ id: 'user-123' }),
      signedUploadRepository: { create },
    });
    const response = await app.request(
      'http://api.test/v1/accounts/account-123/upload-url',
      {
        method: 'POST',
        headers: {
          authorization: 'Bearer session-token',
        ...
        body: JSON.stringify({ fileName: 'activity.csv' }),
      },
    );
    expect(response.status).toBe(201);
    await expect(response.json()).resolves.toEqual({
      upload: {
        bucket: 'brokerage-statements',
        path: 'user-123/account-123/upload.csv',
        token: 'upload-token',
        signedUrl: 'https://storage.test/upload',
      },
  ...
  it('rejects oversized JSON before repository work and preserves a correlation id', async () => {
    ...
    const response = await app.request(
      'http://api.test/v1/accounts/account-123/upload-url',
      {
        ...
        body: JSON.stringify({ fileName: 'x'.repeat(70_000) }),
      },
    );
