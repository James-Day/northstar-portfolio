...
export function createApi(dependencies: ApiDependencies = {}) {
  ...
  const importIssueRepository = dependencies.importIssueRepository;
  ...
  api.get("/v1/accounts", async (context) => {
    ...
    return context.json({
      accounts: await repository.list(
        authenticated.user.id,
        authenticated.accessToken,
      ),
    });
  ...
  api.get("/v1/accounts/:accountId/activity", async (context) => {
    ...
    return context.json({
      activity: await repository.list(accountId, authenticated.accessToken, {
        limit,
        offset,
      }),
    });
  ...
  api.post("/v1/accounts/:accountId/imports", async (context) => {
    ...
    const importRecord = await imports.stage(
      authenticated.accessToken,
      toPersistableImportStage(staged, fileName),
    );
    return context.json(
      {
        import: {
...
function createAccountsRepository(bindings: ApiBindings): AccountsRepository {
  if (!bindings.SUPABASE_URL || !bindings.SUPABASE_ANON_KEY)
...
function createImportsRepository(bindings: ApiBindings): ImportsRepository {
  if (!bindings.SUPABASE_URL || !bindings.SUPABASE_ANON_KEY)
    throw new SessionConfigurationError();
  return new SupabaseImportsRepository({
    supabaseUrl: bindings.SUPABASE_URL,
    supabaseAnonKey: bindings.SUPABASE_ANON_KEY,
  });
}

function createPriceFreshnessRepository(
  bindings: ApiBindings,
): PriceFreshnessReportRepository | undefined {
  if (!bindings.SUPABASE_URL || !bindings.SUPABASE_SERVICE_ROLE_KEY)
    return undefined;
  return new SupabasePriceFreshnessReportRepository({
    supabaseUrl: bindings.SUPABASE_URL,
...
function createReportSnapshotReader(bindings: ApiBindings) {
  ...
  return new SupabaseReportSnapshotReader({
    supabaseUrl: bindings.SUPABASE_URL,
    anonKey: bindings.SUPABASE_ANON_KEY,
  });
}

function createSignedUploadRepository(
  bindings: ApiBindings,
  ...
  return new SupabaseSignedUploadRepository({
    supabaseUrl: bindings.SUPABASE_URL,
    supabaseAnonKey: bindings.SUPABASE_ANON_KEY,
    accounts: new SupabaseAccountsRepository({
      supabaseUrl: bindings.SUPABASE_URL,
      supabaseAnonKey: bindings.SUPABASE_ANON_KEY,
    }),
  });
...
function createOpeningHistoryRepository(
  bindings: ApiBindings,
): OpeningHistoryRepository {
  if (!bindings.SUPABASE_URL || !bindings.SUPABASE_ANON_KEY)
    throw new SessionConfigurationError();
  return new SupabaseOpeningHistoryRepository({
    supabaseUrl: bindings.SUPABASE_URL,
    supabaseAnonKey: bindings.SUPABASE_ANON_KEY,
  });
}

function createImportIssueRepository(
  bindings: ApiBindings,
): ImportIssueRepository {
  if (!bindings.SUPABASE_URL || !bindings.SUPABASE_ANON_KEY)
    throw new SessionConfigurationError();
  return new SupabaseImportIssueRepository({
    supabaseUrl: bindings.SUPABASE_URL,
    supabaseAnonKey: bindings.SUPABASE_ANON_KEY,
  });
}
...
async function requireBillingAccess(
  ...
  | {
      status: 402 | 503;
      body: { error: string; reason?: string; trialEndsAt?: string | null };
    }
  | undefined
> {
