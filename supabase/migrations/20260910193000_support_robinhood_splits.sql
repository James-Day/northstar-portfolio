create unique index if not exists corporate_actions_robinhood_unique on public.corporate_actions (instrument_id, action_date, ratio_numerator, ratio_denominator, source) where source = 'robinhood_csv';

-- Persist the same SHA-256 canonicalization used by the importer.\n-- This migration also replaces commit_import to persist validated Robinhood SPL actions. A fingerprint
