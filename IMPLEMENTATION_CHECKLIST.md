...
| 2026-09-10 | 09 — Evidence-backed correction boundary (partial) | Added a fail-closed price-correction model and resolver that requires evidence and an explicit version, keeps the original DoltHub close intact, and emits a manual-correction revision only when a matching correction exists. | Supabase correction persistence, report dependency wiring, and independent evidence review remain. |
