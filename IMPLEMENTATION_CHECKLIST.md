...
| 2026-09-10 | 03.07 — Explain Robinhood split blockers | `SPL` rows now produce an actionable review message naming the affected ticker and explaining that corporate-action review is required before commit; regression coverage protects the supplied-export behavior. | The split-ratio resolver and persisted corporate-action execution remain open; the supplied CSV still cannot be committed until those are implemented. |
