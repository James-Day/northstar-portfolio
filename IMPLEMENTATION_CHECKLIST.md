...
| 2026-09-10 | 09.04 — Persisted ledger replay boundary (partial) | Added a service-only Supabase replay reader that filters to committed imports, converts exact database values into deterministic FIFO events/opening lots, and exposes activity coverage/source IDs. | Report composition still needs prices, validated actions, opening-history persistence and snapshot queue orchestration. |
| 2026-09-10 | 10.04 — Authenticated activity reader boundary (partial) | Added account-owned, paginated ledger activity API/repository with optional immutable source-row detail and pagination tests. | Activity UI still shows a pending placeholder until this reader is wired into the screen. |
